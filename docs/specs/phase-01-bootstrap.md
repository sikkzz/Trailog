...
- [x] ESLint + Prettier 공통 설정이 `packages/eslint-config` 에 있고 양쪽 앱에서 사용
- [x] TypeScript strict 모드 활성화 (양쪽 앱)
- [x] 커밋 컨벤션 합의됨 (Conventional Commits, prefix 영어 + 본문 한글, husky + commitlint 강제)
- [x] `.gitignore`, `.env.example` 정리됨 (루트 + apps/server + apps/mobile)
