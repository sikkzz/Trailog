...
- [x] **D2 — `(tabs)/map` 골격 + 위치 권한 + 기본 지도 표시** — NaverMap mount + Seoul 시청 initialCamera + `isShowZoomControls`/`isShowScaleBar`/`isShowCompass` + `expo-location.requestForegroundPermissionsAsync` + `getCurrentPositionAsync` + ref `animateCameraTo` + denied banner UI + `let cancelled` lifecycle 패턴.
- [x] **D3 — 백엔드 bbox 쿼리 API + 모바일 hook** — `GET /photos/map?bbox=minLng,minLat,maxLng,maxLat` (PostGIS `ST_Within` + `ST_MakeEnvelope` + GIST 인덱스 자연 활용, EXPLAIN `Index Scan ~4.5ms`) + `fetchMapPhotos` + `useMapPhotos(bbox)` React Query + `keepPreviousData`(flicker 방지) + `Bbox` tuple type.
- [x] **D4 — 사진 marker + 클릭 navigation** — `onCameraIdle` 자체 debounce → region → bbox 변환 → `useMapPhotos` trigger → `<NaverMapMarkerOverlay>` 자식 렌더 → `onTap` 클로저 → `router.push` photo detail (photoId + momentId query).
- [x] **D5 — Cluster** — NaverMap 자체 `clusters` prop 활용. children marker → `ClusterMarkerProp[]` 변환 + `screenDistance: 100px` + zoom 0~16 cluster + 17+ 풀림 + `onTapClusterLeaf` 콜백 (markerIdentifier로 photo find).
- [x] **D6 — 사진 상세 미니맵 + reverse geocoding** — NaverMap 정적 미니맵 (gesture 모두 비활성) + 단일 marker. **OS별 reverseGeocode 결과 차이 발견** (iOS Apple "서울특별시 태평로1가 세종대로 서울특별시청" / Android Google "서울특별시 중구 태평로1가 31") → **NCP Reverse Geocoding 백엔드 proxy 도입** (`apps/server/src/geocoding/` + `apps/mobile/src/lib/geocoding/` + `useReverseGeocode` staleTime Infinity). Client Secret 백엔드 only.
- [x] **D7 — 학습 노트 3건** — [mobile-map-libraries-comparison.md](../learnings/mobile-map-libraries-comparison.md) (네이버맵 채택 흐름 + 6 lib 시장 비교 + supersede 흐름) + [cluster-algorithms-and-spatial-queries.md](../learnings/cluster-algorithms-and-spatial-queries.md) (Grid/DBSCAN/Supercluster + NaverMap built-in + PostGIS bbox 흐름 + GIST 인덱스) + [naver-cloud-platform-reverse-geocoding.md](../learnings/naver-cloud-platform-reverse-geocoding.md) (NCP API + OS별 Geocoding 통일 + 백엔드 proxy 패턴 + Client Secret 보안).

검증: typecheck/lint pass + iOS Simulator(서울 시청 marker + photo detail 미니맵 + 한국어 주소) + Android Emulator(위치 mock 한계로 일부 검증은 후속) + DB PostGIS ST_Within EXPLAIN Index Scan 확인 + NCP curl 직접 검증.
...
**Android Emulator 위치 mock 함정 (D2 디버깅 박제)**: `adb emu geo fix`는 GPS provider만 mock, `expo-location`은 fused_location_provider 사용 → cache(Google HQ) 반환. iOS Simulator는 Custom Location preferences 영구 저장(전체 reload OK). Android Emulator는 영구 default 박기 어려움 — Quick Boot lock 또는 코드 dev mock 우회. 자세히는 [react-native-fundamentals-for-web-devs.md 2026-06-07 추가분](../learnings/react-native-fundamentals-for-web-devs.md).
...
| 2026-06-07 | **🎉 4.7 지도 표시 완료** — D2(NaverMap mount + Seoul initialCamera + expo-location 권한 + animateCameraTo + denied banner UI) + D3(백엔드 `GET /photos/map?bbox=...` + PostGIS `ST_Within` + `ST_MakeEnvelope` + GIST 인덱스 자연 활용 EXPLAIN `Index Scan ~4.5ms` + 모바일 `useMapPhotos(bbox)` hook + `keepPreviousData` flicker 방지) + D4(`onCameraIdle` → bbox → marker overlay + `onTap` photo detail navigation) + D5(NaverMap 자체 `clusters` props + ClusterMarkerProp 배열 + `screenDistance: 100px` + zoom 0~16 + `onTapClusterLeaf`) + D6(사진 상세 미니맵 정적 NaverMap + 단일 marker + **NCP Reverse Geocoding 백엔드 proxy** — OS별 차이 발견 후 도입). **OS별 reverseGeocode 차이 발견 + NCP 통일** — iOS Apple Geocoding "서울특별시 태평로1가 세종대로 서울특별시청" vs Android Google Geocoding "서울특별시 중구 태평로1가 31"이라 같은 좌표 다른 form. `apps/server/src/geocoding/` (Service + Controller + DTO + AppModule 등록) + `apps/mobile/src/lib/geocoding/` (Schema + fetchReverseGeocode + useReverseGeocode staleTime Infinity). NCP_CLIENT_SECRET은 백엔드 only(.env, gitignored). 신규 게이트웨이 `maps.apigw.ntruss.com` 사용. **Android Emulator 위치 mock 함정 박제** — `adb emu geo fix`는 GPS provider만 mock, expo-location은 fused_location_provider 사용 → Google HQ cache 반환. RN fundamentals 학습 노트 2026-06-07 추가 박제. **학습 노트 3건 신규** — [mobile-map-libraries-comparison.md](../learnings/mobile-map-libraries-comparison.md) + [cluster-algorithms-and-spatial-queries.md](../learnings/cluster-algorithms-and-spatial-queries.md) + [naver-cloud-platform-reverse-geocoding.md](../learnings/naver-cloud-platform-reverse-geocoding.md). 메모리 박제 1건 — [mobile-map-library-revisit](./memory) (글로벌 출시/lib 정체/SDK 메이저 업그레이드/사용자 100명+ 시점 능동 알림 + NCP Client ID git 박힘 → Phase 4 env 분리 추후 인지). 다음: 4.8 UI/UX 폴리시 wave 또는 Phase 3 (공유). |
