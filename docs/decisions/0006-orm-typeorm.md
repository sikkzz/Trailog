...
**선택**: ✅ **TypeORM** (1.0.x — 2026-04 메이저 출시, 도입 시점 install)

세부:

- `@nestjs/typeorm` + `typeorm@1.0` 도입
- DataMapper 패턴 (NestJS 정석, Repository injection)
- 마이그레이션: TypeORM CLI (`typeorm migration:generate / run / revert`) via `typeorm-ts-node-commonjs`
- PostGIS: `@Column({ type: 'geometry', ... })` + 필요시 raw SQL
- 위치 쿼리: PostGIS 함수 raw SQL (`ST_DWithin`, `ST_GeomFromText` 등)

**버전 메모** (2026-05-28 정정):

- 본 ADR 초안엔 "0.3.x"로 적었으나 실제 install은 `typeorm@^1.0.0` 들어옴 (npm 레지스트리 1.0 출시 직후 시점).
- TypeORM 1.0은 0.3에서 API 변경 거의 없는 메이저 (semver 정리 측면) — 코드 변경 X, ts-node CLI 호환은 Node 20.19+ 필요 (require-ESM 지원).
- ⚠️ Node 20.14 같은 옛 버전에선 `ERR_REQUIRE_ESM` (yargs ESM 충돌). 본 프로젝트 `.nvmrc=20.19.4` 명시로 해결.
...
- [x] Q1 결정 (TypeORM)
- [x] ADR-0006 작성 (본 문서)
- [x] Phase 2 spec Q1 박제
- [x] PROJECT_ROOT 4장 + 11장 갱신
- [x] `@nestjs/typeorm`, `typeorm`, `pg`, `@nestjs/config`, `dotenv` 의존성 추가
- [x] DataSource + DatabaseModule 셋업
- [x] User 엔티티 + 첫 마이그레이션 (`1779978806585-Init.ts`)
- [x] 학습 노트 `typeorm-deep-dive.md` (참조 패턴 비교 + 추후 정복 추적표)
- [x] 학습 노트 `postgres-vs-mysql.md` (PostGIS 사유 + 참조 MySQL과 의도적 다양성)
- [ ] 마이그레이션 GitHub Actions deploy 자동 실행 (Phase 2 4.2 진입 시)
- [ ] entity 5개+ 도달 시 도메인별 spread 패턴 (메모리 인지)
