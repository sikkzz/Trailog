...
**학습 영역: 인프라 / DevOps (2차 — 운영 관점 + AWS 실무 스택)** + #6 모바일 스토어 배포
...
> **자세한 spec**: [Phase 4 Spec Draft (2026-07-03)](./specs/phase-04-operations-and-aws-migration.md).
...
| 2026-07-03 | **🎯 Phase 4 spec draft 작성** ([Phase 4 Spec](./specs/phase-04-operations-and-aws-migration.md)). Phase 3 종료 직후 진입 준비. **3축 구조**: 관측 가능성(Sentry + 구조화 로깅 + Uptime + 비용 알람) + 스토어 배포(Apple Developer + Google Play + EAS Submit + TestFlight/Play 내부 트랙) + AWS ECS Fargate 마이그레이션(VPC + RDS + ElastiCache + ECR + Task Definition + ALB + Route 53 + ACM + 선택 Terraform IaC). Phase 3 이월 항목 통합 — SSE Heartbeat/Redis Pub-Sub(sse-phase4-enhancements-revisit), 알림 영속화/푸시(5.3 이월), NCP env 분리(mobile-map-library-revisit), RestResponse layer(error-handling-revisit), Stateful logout(auth-deep-dive-revisit), BullMQ 위치 리팩토링(bullmq-domain-vs-root-revisit), Notion sync 정착(notion-sync-rearchitect-revisit). Q1~Q10 미정 사안(로깅 lib / Uptime 서비스 / Sentry / DB 이전 시점 / Terraform 여부 / 도메인 등록 / 스토어 자산 / 푸시 provider / 알림 스키마 / 이용약관). wave 8개(Q → 6.1 관측 → 6.2 CI/CD → 6.3 알림 → 6.4 스토어 → 6.5 AWS 준비 → 6.6 ECS → 6.7 IaC → 6.8 컷오버) + 4~6주 호흡. 성공 지표 정직 측정 시작(Uptime 99%, 5xx 0.1%, P50/P95, 비용 $80).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 2026-07-04 | **🎉 1차 마무리 완료** ([ADR-0017](./decisions/0017-first-milestone-scope.md)) — Phase 3 4개 wave 완주 + 문서 정리 마감. 시작(2026-05-21) 대비 약 6.5주. 9장 성공 기준 도달 여부 정직 명시 — **최소 성공(Must) ✅ 달성** (실 디바이스 실사용 + 학습 영역 4개+ + 정리된 코드/README) / **중간 성공(Should) 부분 달성** (학습 영역 4/6 완주 · 지인 베타 5명+ 및 블로그 3편은 미달). Phase 4(스토어 배포 + AWS ECS)는 실비 트레이드오프(Apple $99/년 + Google $25) + 트래픽 미확보로 오버킬 판단 → 2차 확장 후보로 박제(spec 보존). Trip wave도 필수 X + 개발 wave 확장 최소화 원칙으로 보류. **2차 확장 후보 5개** — Trip+타임라인 / 스토어 배포 / 관측 인프라 최소(Sentry) / AWS ECS 마이그레이션 / 사진 편집·검색·태그. 재활성 트리거 조건은 각 spec에 박제. 블로그 정리는 본인 별도 repo(기술 블로그 자동화 프로젝트)에서 자연 페이스로 이어감. 사이드 정직 브레이크 진입.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
