...
### Phase 3: 사진 공유 + 동행자 + 권한 모델 (3~4주)

> **2026-06-09 reshape**: 원 Phase 3 정의(지도 + 시각화)는 Phase 2 4.7에서 지도 표시 + 클러스터링 흡수 완료. Trip 단위 + polyline + 타임라인 슬라이더는 **Phase 3 종료 후 별도 wave**로 분리 (학습 영역 #3 시각화 깊이). 자세히: [Phase 3 Spec](./specs/phase-03-sharing.md).

**학습 영역: 실시간 통신 (SSE/WebSocket) + 권한 모델 + 이미지 보안 (EXIF strip)**

- 동행자 초대 (이메일/사용자 ID) + Moment 권한 모델 (owner/contributor/viewer)
- 공유 링크 (만료 + 비밀번호 + 외부 사용자 read-only)
- EXIF GPS strip (프라이버시 — 공유 시 GPS 누출 방지)
- 실시간 알림 (SSE 또는 WebSocket — Q1)
- 푸시 알림 (Expo Notifications — Q2, Phase 3 또는 4 결정)
- 백엔드 RBAC + NestJS Guard 패턴 (참조 다층 가드 9개 비교 — 메모리 `auth-deep-dive-revisit` 활성화)

**산출물**: 혼자 박제하던 Moment를 동행자와 공유하고, 외부 사람에게도 공유 링크로 보여주고, GPS 정보 strip으로 프라이버시 지킬 수 있는 흐름.

### Phase 3 종료 후 별도 wave: Trip + 시각화 깊이 (잠정)

- 여행(Trip) 단위로 Moment 묶기
- 시간순 polyline 루트 표시
- 타임라인 슬라이더로 시간대별 필터링
- 갤러리 인터랙션 + Bottom Sheet
- 학습 영역 #3 (시각화 깊이) — Reanimated + 인터랙션 폴리시
